...
- ✅ An experienced technical leader sharing battle-tested advice
...
❌ **Vague**: "Many mainframe engineers struggle with using AI."
⚠️ **Generic**: "Modernization leaders often find it difficult to modernize applications."
✅ **Specific**: "When Sarah started working on business rule extraction to reimagine part of the banks application, she spent 3 months waiting for subject matter expert help to analyze the code."
🏆 **Compelling**: "When Sarah started working on business rule extraction to reimagine part of the banks application, she spent 3 months waiting for subject matter expert (SME) help to analyze the code until she uncovered that new tools could augment her SME turning months into days."
...
**Before**: "It's important to consider the complexity of your application."
**After**: "You may (not) find it shocking but mainframe applications are confusing to the average developer (If they weren't then the remaining mainframe engineers wouldn't make so much money.)"
...
**Monotonous**:
"You need to understand the codebase. You should trace dependencies. You must map the architecture. You can't skip context."

**Varied**:
"Start by understanding the codebase. Then trace dependencies—where does this function actually lead? (And what breaks if you touch it?) Map the architecture so nothing surprises you. Skip context at your peril."
...
**Actionable List**:
- **Code exploration**: Open the service you're debugging. Trace the entry point to the database call—count how many files you touch.
- **Dependency mapping**: Find every service that calls your API.
- **Architecture understanding**: Draw the data flow from user click to database write. Can't do it? That's your first priority.
- **Knowledge sharing**: Document one "why" today—not what the code does, but why it was built this way.
...
**Before**:
"Legacy modernization requires understanding your existing systems and extracting business rules over time."

**After**:
"One enterprise estimated six months for COBOL business rule extraction. Three senior engineers spent weeks explaining payment validation logic line-by-line while analysts waited. Eighteen months later, they'd documented 40% of the application. An application understanding platform surfaced all the business rules in one day—traced from screen input through 15 interconnected programs to database update."
...
**Before**:
"When it comes to mainframe modernization, it's important to note that understanding is key. Furthermore, business rule extraction matters significantly. Additionally, tribal knowledge should not be overlooked."

**After**:
"Want to modernize your mainframe? Understanding comes first. (Obvious, right?) But here's what most programs miss: Understanding without completeness is dangerous. And tribal knowledge locked in your senior engineers' heads? That's a ticking clock—they're retiring whether you're ready or not."
...
**Before**:
"You should document your legacy systems to improve modernization outcomes."

**After**:
"Pick your most critical COBOL program right now. Trace one business rule from the input screen to the database write. Count how many programs you touch, how many SMEs you interrupt, how many hours it takes. That's your modernization bottleneck—and exactly what automated extraction eliminates."
...
5. **Stay On Brand**: Maintain brand voice from `context/brand-voice.md`
