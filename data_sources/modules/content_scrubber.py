...
class ContentScrubber:
    ...
    def _fix_url_spaces(self, content: str) -> str:
        """
        ...
        def fix_url(match):
            ...
            # Remove spaces after dots within the URL
            cleaned = re.sub(r'\. (?=[a-zA-Z0-9_\-])', '.', url)
            # Remove spaces after ? in query strings
            cleaned = re.sub(r'\? (?=[a-zA-Z0-9_\-])', '?', cleaned)
            # Remove spaces after = in query params
            cleaned = re.sub(r'= (?=[a-zA-Z0-9_\-])', '=', cleaned)
            # Remove spaces after & in query params
            cleaned = re.sub(r'& (?=[a-zA-Z0-9_\-])', '&', cleaned)
            # Remove spaces after / in paths
            cleaned = re.sub(r'/ (?=[a-zA-Z0-9_\-])', '/', cleaned)
            # Remove spaces after : in protocol
        ...
        # Match URLs inside markdown links: [text](url) and ![alt](url)
        content = re.sub(
            r'(\]\()([^)]+)(\))',
            fix_url,
            content,
        ...
        self.stats['url_spaces_fixed'] = fixed_count
        return content
    ...
    def _clean_whitespace(self, content: str) -> str:
        ...
        # Fix spacing around punctuation (excluding dots, which are ambiguous
        # in URLs, file extensions like .png, and technology names like .NET)
...
def scrub_content(content: str, verbose: bool = False) -> str:
    ...
    if verbose:
        ...
        print(f"  - URLs with spaces fixed: {stats['url_spaces_fixed']}")
