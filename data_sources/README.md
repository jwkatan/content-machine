...
- **Content Gaps**: Topics competitors rank for but you don't
...
### WordPress
- **Purpose**: Content import/export with Gutenberg block preservation
- **Key Features**:
  - Import articles from WordPress URLs
  - Export edited content back to WordPress (as drafts)
  - Preserve Gutenberg block structure during round-trips
  - Upload images to WordPress media library
  - Block registry for 203+ block types
...
```python
from data_sources.modules.wordpress_client import (
    import_from_wordpress,
    export_to_wordpress,
    regenerate_markdown
)

# Import an article (preserves Gutenberg blocks)
success, folder = import_from_wordpress(
    "https://yoursite.com/blog/article-slug/"
)
# Creates: published/{slug}/original.html, article.md, metadata.json

# Edit original.html using /wordpress-edit skill...

# Regenerate markdown after edits
regenerate_markdown(folder)

# Preview export (no actual push)
success, preview = export_to_wordpress(folder)
print(preview)

# Export to WordPress as draft
success, result = export_to_wordpress(folder, push=True)
```
